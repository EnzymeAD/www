...
def rs_get_resource(path):  # pragma: no cover
    try:
        import sys
        #print(request.headers, file=sys.stderr)
        resp = requests.get('https://enzymead.github.io/rustbook/'+path, allow_redirects=False) # , headers=request.headers)
        excluded_headers = ['content-encoding', 'content-length', 'transfer-encoding', 'connection']
        headers = [(name, value) for (name, value) in resp.raw.headers.items() if name.lower() not in excluded_headers]
        #print(headers,file=sys.stderr)
        res = Response(resp.content, resp.status_code, headers)
...
def dox_get_resource(path):  # pragma: no cover
    try:
        import sys
        # print(request.headers, file=sys.stderr)
        resp = requests.get('https://enzymead.github.io/Enzyme/'+path, allow_redirects=False) # , headers=request.headers)
        excluded_headers = ['content-encoding', 'content-length', 'transfer-encoding', 'connection']
        headers = [(name, value) for (name, value) in resp.raw.headers.items() if name.lower() not in excluded_headers]
        # print(headers,file=sys.stderr)
        res = Response(resp.content, resp.status_code, headers)
